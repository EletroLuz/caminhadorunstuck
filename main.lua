...
local unstuck = require("data.unstuck") -- Adicione esta linha
...
local function pulse()
    ...
    if current_waypoint then
        ...
        if distance < 1 then
            ...
            -- Verifica se o jogador está preso por mais de 15 segundos
            if unstuck_start_time == nil then
                unstuck_start_time = current_time
            elseif current_time - unstuck_start_time > 15 then
                console.print("Player stuck for 15 seconds, calling unstuck script")
                unstuck.initiate_pathfinding(player_pos, current_waypoint, 50)
                unstuck_start_time = nil -- Reseta o tempo de início do unstuck
            end
        end
    end
end
